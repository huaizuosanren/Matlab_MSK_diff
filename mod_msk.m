function[signal_out,I_out,Q_out]=mod_msk(data,data_len,sample_number,Rb)%MSK基带调制??
%**************************************************************************??
%?data??????????????调制信号??%?data_len??????????码元个数??
...
%?plot(I1,'.-');title('加权前I路');xlabel('时间');ylabel('幅度');??%?subplot(224)??
%?plot(Q1,'.-');title('加权前Q路');xlabel('时间');ylabel('幅度');?
%???%?figure(2)??%?subplot(221)??
%?plot(cos(pi*t/2/Tb),'.-');title('加权函数cos(πt/(2Tb))');xlabel('时间');ylabel('幅度')??%?subplot(222)??
%?plot(sin(pi*t/2/Tb),'.-');title('加权函数sin(πt/(2Tb))');xlabel('时间');ylabel('幅度')??%?subplot(223)??
%?plot(I_out,'.-');title('加权后I路');xlabel('时间');ylabel('幅度');??%?subplot(224)??
